.productMsg <- function(valuesTPG, matrixFunction, matrixNode, index1, index2){
    ...
    for (z in seq_len(length(valuesTPG)-1)) {
        valuesMsg <- matrixNode[matrixFunction[index1,
                     index2][[1]]$neighbor[z], index1][[1]][[valuesTPG[z]]]
        valueProduct <- valuesMsg * valueProduct}
    return(valueProduct)}

msgFGGA <- function(matrixFGGA, obsValueGOs, graphGO, tmax=200, epsilon=0.001){
    if (is(matrixFGGA) != "fgga"){
        print("matrixFGGA is not class fgga")
        return()
    } else { matrixFGGA <- as(matrixFGGA, "matrix")}
    ...
    names(functionsTPG) <- sapply(nParents, FUN=function(att) (paste("TPG",
                                  att, sep="", collapse='')))
    #Initialization
    for (i in seq_len(dim(matrixFunctionToNode)[2])){
        matrixFunctionToNode[nNodes-1+i, i][[1]] <- list(
        zeroValues=1-obsValueGOs[i], oneValues=obsValueGOs[i], valueGO =1)
        if (i < nNodes) {nodeLinks <- which(matrixFGGA[,i] == 1)
        ...
        for (k in seq_len(length(nodeLinks)))
            if (length(nodeLinks) > 2) matrixFunctionToNode[i, nodeLinks[k]][[1]] <-
            list(zeroValues=1, oneValues=1, neighbor=functionLink[k, ],
            leafGO=leafGO, parentsGO=parentsGO, TPG=length(nodeLinks)-1) else
            matrixFunctionToNode[i, nodeLinks[k]][[1]] <- list(zeroValues=1,
            oneValues=1, neighbor=functionLink[1, k], leafGO=leafGO,
            parentsGO=parentsGO, TPG=length(nodeLinks)-1)}
        nodeLinks <- which(matrixFGGA[i,] == 1)
        functionLink <- t(sapply(names(nodeLinks), FUN=function(x, y) (setdiff(
                          y, x)), y=names(nodeLinks)))
        for (k in seq_len(length(nodeLinks)))
            if (length(nodeLinks) > 2) matrixNodeToFunction[i,
                nodeLinks[k]][[1]] <- list(zeroValues=1, oneValues=1, neighbor=
                                             functionLink[k, ]) else
    ...
    for (t in seq_len(tmax)){
        ...
        for(i in seq_len(nNodes)){
            ...
            for (j in seq_len(length(activeFunction))){
                valuesMsg <- apply(t(activeFunction[-j]), MARGIN=2, FUN=
                             function(x, y, z) (y[x, z][[1]]$oneValues),
                             y=matrixFunctionToNode, z=i)
                matrixNodeToFunction[i, activeFunction[j]][[1]]$oneValues <-
                    prod(valuesMsg)
                valuesMsg <- apply(t(activeFunction[-j]), MARGIN=2, FUN=
                            function(x, y, z) (y[x, z][[1]]$zeroValues),
                            y=matrixFunctionToNode, z=i)
                    ...
                    for (k in seq(1,2)) {
                        ...
                        if (k == 1) matrixFunctionToNode[i,
                          ...
                          sum(valuesMsg) else
                                ...
                                sum(valuesMsg)}} else {
                                for (k in seq(1,2)) {
                                    indexMsg <- which(functionsTPG[[
                                    matrixFunctionToNode[i, activeFunction[j]
                                    ][[1]]$TPG]][, 2] == k)
                                    subMatrixTPG <- functionsTPG[[
                                    matrixFunctionToNode[i,
                                    activeFunction[j]][[1]]$TPG]][indexMsg, -2]
                                    if (is.matrix(subMatrixTPG)) valuesMsg <-
                                      apply(subMatrixTPG, MARGIN = 1,
                                      FUN = .productMsg, index1 = i,
                                      index2 = activeFunction[j],
                                      matrixFunction = matrixFunctionToNode,
                                      matrixNode = matrixNodeToFunction) else
                                      valuesMsg <- apply(as.matrix(t(
                                        subMatrixTPG)), MARGIN = 1,
                                        FUN = .productMsg, index1 = i, index2 =
                                        activeFunction[j],
                                        matrixFunction = matrixFunctionToNode,
                                         matrixNode = matrixNodeToFunction)
                                    if (k == 1) matrixFunctionToNode[i,
        ...
        oneValuesGO <- sapply(seq(1, nNodes), FUN=function(x, y, z)
            (y[[x]]$oneValues * z[x]), y=valuesGO, z=obsValueGOs)
        zeroValuesGO <- sapply(seq(1,nNodes), FUN=function(x, y, z)
            (y[[x]]$zeroValues * (1-z[x])), y=valuesGO, z=obsValueGOs)
        oneValuesGO <- sapply(seq(1,nNodes), FUN=function(x, y, z)
            (y[x]/(y[x] + z[x])), y=oneValuesGO, z=zeroValuesGO)
        valuesGO <- sapply(diag(matrixFunctionToNode[seq(nNodes,nFactors), ]),
                          FUN=function(x) (x$valueGO))
        errorGO <- sapply(seq(1,nNodes), FUN=function(x, y, z) (abs(y[x] -
                          z[x])), y=oneValuesGO, z=valuesGO)
        if (max(errorGO) < epsilon) {
            names(oneValuesGO) <- colnames(matrixFunctionToNode)
            return(oneValuesGO)} else {for (i in seq_len(nNodes))
