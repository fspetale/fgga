  ...
  if (k_fold < length(target)) {
    ...
    for (i in seq_len(length(numInClass))) {
      ...
      if (min_reps > 0) {
        ...
        if (spares > 0) seqVector <- c(seqVector, sample(seq_len(k_fold),
                                                         spares))
        foldVector[which(target == names(numInClass)[i])] <- sample(seqVector)
      }
      else {
        foldVector[which(target == names(numInClass)[i])] <- sample(seq_len(
          k_fold), size = numInClass[i])
      }
    }
  }
