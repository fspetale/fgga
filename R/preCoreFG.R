.goPlusList <- function(relationship, goPlus){
  goRelation <- lapply(goPlus, FUN=function(x) (x[intersect(intersect(grep(
    relationship, x$pred), grep("GO_", x$sub)), grep("GO_", x$obj)), ]))
  indexList <- which(lapply(goRelation, FUN = function(x) (length(x$obj)))>0)
...
.goPlusInfo <- function (){
  ...
  goPlus <- sapply(codeRelations, FUN = .goPlusList,
                   goPlus = goTermInfo$graphs$edges)
  indexGOTerms <- seq(2, length(nameRelations))
  goPlusEdges <- lapply(seq_len(length(nameRelations)), FUN = function(x, y, z)
    {x[[z]][, 2] <- y[z]; return(x[[z]])}, x = goPlus, y = nameRelations)
  goPlusMatrixEdges <- do.call("rbind", goPlusEdges)
  goPlusMatrixEdges <- cbind(goPlusMatrixEdges,
                             matrix(0, dim(goPlusMatrixEdges)[1], 3))

  goTerms <- sapply(goTermInfo$graphs$nodes, FUN = function(x)
    (gsub("http://purl.obolibrary.org/obo/GO_", "GO:", x$id)))
  goDomains <- sapply(goTermInfo$graphs$nodes[[1]]$meta$basicPropertyValues,
                      FUN = function(x) {
    y <- grep("biological_process|molecular_function|cellular_component", x$val)
    if (length(y) > 0) return (
      switch(x$val[y], "biological_process"="BP", "molecular_function"="MF",
             "cellular_component"="CC")) else return("")})
  ...
  goPlusMatrixEdges[indexGOTerms, 4] <- sapply(goPlusEdges[[i]][
    seq_len(goRelationLength)], FUN = function(x, y) {y[x, 2]}, y = goTerms)
  if (i == 1) {goPlusMatrixEdges[indexGOTerms, 5] <- goPlusMatrixEdges[
    indexGOTerms, 4]} else {
    goPlusMatrixEdges[indexGOTerms, 5] <- sapply(
      goPlusEdges[[i]][seq.int(2 * goRelationLength + 1, 3 * goRelationLength)],
      FUN = function(x, y) {y[x, 2]}, y = goTerms)}
  goPlusMatrixEdges[indexGOTerms, 6] <- paste(
    goPlusMatrixEdges[indexGOTerms, 4], goPlusMatrixEdges[indexGOTerms, 5],
    sep = "-")
  }

  colnames(goPlusMatrixEdges) <- c("Son", "Relationship", "Father", "Domain_Son",
...
.readGO <- function (file){
  ...
  tagMatches <- regmatches(x = lines[taggedLines],
                           regexec(text = lines[taggedLines],
                                   pattern = tagRegexp))
  tags <- sapply(tagMatches, "[", 3)
  values <- sapply(tagMatches, "[", 4)
  allTagTypes <- unique(tags)
  useTags <- if (minimal)
    intersect(c("id", "name", "is_obsolete"), allTagTypes)
  else allTagTypes
  propagateLines <- which(tags %in% propagate_relationships)
  parents <- unname(lapply(FUN = unique, split(values[propagateLines],
                    cut(taggedLines[propagateLines], breaks = c(termLines, Inf),
  ...
  properties <- mapply(SIMPLIFY = FALSE, FUN = function(vals, lns) {
    unname(split(vals, cut(lns, breaks = c(termLines, Inf), labels =
                             seq(length(termLines)))))
  }, split(values[tagLines], tags[tagLines]), split(taggedLines[tagLines],
                                                    tags[tagLines]))
  simplify <- intersect(names(properties), c(
  "id", "name", "def", "comment", "is_obsolete", "created_by", "creation_date"))
  ...
  do.call(what = .ontologyIndex, c(list(version = substr(
    ...
    obsolete = if ("is_obsolete" %in% names(properties)) (!is.na(
      properties[["is_obsolete"]])) & properties[["is_obsolete"]] == "true" else
        rep(FALSE, length(properties[["id"]]))), properties[
          -which(names(properties) %in% c("id", "name", "is_obsolete"))]))
}

.strAncsFromPars <- function(id, pars, chld) {
	stopifnot(all(sapply(list(pars, chld), function(x) is.null(names(x)) |
	                       identical(names(x), id))))
	int.pars <- c(split(as.integer(
	  factor(unlist(use.names = FALSE, pars), levels = id)), unlist(
	    use.names = FALSE, mapply(SIMPLIFY=FALSE, FUN=rep, id, sapply(
	      pars, length)))), setNames(nm = setdiff(id, unlist(
	...
	int.chld <- c(split(as.integer(
	  factor(unlist(use.names = FALSE, chld), levels = id)),unlist(
	    use.names = FALSE, mapply(SIMPLIFY = FALSE, FUN=rep, id, sapply(
	      chld, length)))), setNames(nm = setdiff(id, unlist(
	        use.names=FALSE, chld)), rep(list(integer(0)), length(setdiff(
	          id, unlist(use.names=FALSE, chld))))))[id]

  setNames(nm = id,
           lapply(.ancsFromPars(int.pars, int.chld), function(x) id[x]))
}
          ...
          obsolete = setNames(nm = id, rep(FALSE, length(id))), version = NULL)
{
  ...
  if (length(missing_terms) > 0) {
    warning(paste0("Some parent terms not found: ",
    paste0(collapse = ", ", missing_terms[seq(min(length(missing_terms), 3))]),
  if (length(missing_terms) > 3)
    paste0(" (", length(missing_terms) - 3, " more)") else ""))
    parents <- lapply(parents, intersect, id)
  }
  children <- c(lapply(
    FUN = as.character, X = split(unlist(
      use.names = FALSE, rep(id, times = sapply(parents, length))),
      unlist(use.names = FALSE, parents))),
    setNames(nm = setdiff(id, unlist(use.names = FALSE, parents)),
             rep(list(character(0)), length(setdiff(
               id, unlist(use.names = FALSE, parents))))))[id]
  structure(lapply(
    FUN = setNames, nm = id,
    X = list(id = id, name = name, parents = parents, children = children,
...
preCoreFG <- function(myGOnames, domains="goPlus"){
  ...
  nameRelations <- c("is_a", "part_of", "occurs_in", "regulates",
                  "negatively_regulates", "positively_regulates", "capable_of",
                  "capable_of_part_of")
  tableInference <- data.frame(unlist(lapply(nameRelations, FUN = function(x, y)
    rep(x, y), y = length(nameRelations))), rep(nameRelations,
                                              length(nameRelations)), 0)
  colnames(tableInference) <- c("fatherRelation", "grandfatherRelation", "h")
  ontologyGO <- .readGO("http://purl.obolibrary.org/obo/go.obo")
  tableInference[c(seq_len(11), 17, 18, seq.int(24, 26),
                   33, 34, 41, 42, 49, 50, 57, seq.int(60, 62)), 3] <- 1
  ...
  if (domains=="goPlus") indexGOs <- unique(unlist(
    sapply(myGOnames, FUN = function(x, y) which(y[,1] == x), y = goPlusRef))
    ) else{
    gominPlusRef <- rbind(goPlusRef[which(goPlusRef[,6] == crossOntology[1]), ],
                          goPlusRef[which(goPlusRef[,6] == crossOntology[2]), ])
    indexGOs <- unique(
      unlist(sapply(myGOnames, FUN = function(x, y) which(y[,1] == x),
                    y = gominPlusRef[, ])))}
      ...
      for (j in seq_len(length(nodesGO))){
        ...
        if (length(indexGOs) > 0){
          for (j in seq_len(length(indexGOs))){
            if (goPlusRef[indexGOs[j], 4] != goPlusRef[indexGOs[j], 5]){
              if(length(which(myGOnames == goPlusRef[indexGOs[j], 3])) == 1){
                matrixGO[goPlusRef[indexGOs[j], 3],
                         goPlusRef[indexGOs[j], 1]] <- 1
                relationGOs <- goPlusRef[which(goPlusRef[indexGOs[j], 3] ==
                                                 goPlusRef[,1]), ]
                if (is.matrix(relationGOs)) {
                  for (k in seq_len(dim(relationGOs)[1])){
                  hValue <- tableInference[intersect(
                    which(tableInference[, 2] == relationGOs[k, 2]),
                    which(tableInference[, 1] == goPlusRef[indexGOs[j], 2])), 3]
                  if (hValue == 0) matrixGO[relationGOs[k, 1],
                                            goPlusRef[indexGOs[j], 3]] <- 0
                }} else {
		hValue <- tableInference[intersect(
		  which(tableInference[, 2] == relationGOs[2]),
		  which(tableInference[, 1] == goPlusRef[indexGOs[j], 2])), 3]
                  if (hValue==0) matrixGO[relationGOs[1],
                                          goPlusRef[indexGOs[j], 3]] <- 0
                }
  ...
  if (domains=="BP-MF")  matrixGO["GO:FES", c("GO:0008150", "GO:0003674")] <- 1
  if (domains=="BP-CC")  matrixGO["GO:FES", c("GO:0008150", "GO:0005575")] <- 1
  if (domains=="MF-CC")  matrixGO["GO:FES", c("GO:0003674", "GO:0005575")] <- 1
  if (domains=="goPlus") matrixGO["GO:FES", c("GO:0008150", "GO:0003674",
                                              "GO:0005575")] <- 1
