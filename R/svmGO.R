  ...
  if ((np != 0) && (nn != 0)) {
    if (nn > 2*np || np > 2*nn) {
      if (nn > 2*np){
        ...
        exprsValues <- dxCharacterized[c(positiveGeneNames,
                  negativeGeneNames[id.negative]), ]}
      else{ np <- 2*nn
           id.positive <- sample(seq_len(length(positiveGeneNames)), np)
           exprsValues <- dxCharacterized[c(positiveGeneNames[id.positive],
                                                  negativeGeneNames), ]}}
    else{ exprsValues <- dxCharacterized[c(positiveGeneNames,
                                           negativeGeneNames), ]}
    ...
    modelSvm <- svm(exprsValues, gos, method = "C-classification", cost = 500,
...
svmGO <- function(svmMoldel, dxCharacterized, rootNode, varianceSVM){
  ...
  matrixTest <- sapply(dxTestSVM, cbind)
  colnames(matrixTest) <- sapply(dxTestSVM, colnames)
  rownames(matrixTest) <- sapply(dxTestSVM[1], rownames)
  return(matrixTest)}
...
.svmTest <- function(svmMoldel, dxTest, rootGO, variance){
  ...
  if (nodeGO != rootGO){
    ...
    predGO <- predict(svmMoldel, dxTest, decision.values=T)
    matrixScore[, 1] <- 1/(1 + exp(-2*attr(predGO, "decision.values")/
                                   variance[nodeGO]))
  } else matrixScore[, 1] <- 0.9999
